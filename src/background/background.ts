import { deleteDocument, listDocuments, putChunks, putDocument, type RagChunk, type RagDocument, getChunksForDocs } from "./ragDb";
...
type RagIndexResponse = { ok: true; docId: string; chunkCount: number } | { ok: false; error: string };

type RagListDocsRequest = { type: "RAG_LIST_DOCUMENTS" };
type RagListDocsResponse = { ok: true; documents: RagDocument[] } | { ok: false; error: string };
...
type RagSearchResponse = { ok: true; hits: RagSearchHit[] } | { ok: false; error: string };

type AnyRequest = OpenRouterChatRequest | RagIndexRequest | RagListDocsRequest | RagDeleteDocRequest | RagSearchRequest;

async function openRouterEmbeddings(apiKey: string, inputs: string[]): Promise<number[][]> {
  ...
  for (const item of data) {
    ...
    if (!Array.isArray(emb)) throw new Error("Embeddings item missing embedding[]");
    out.push(emb as number[]);
  }
  return out;
}

async function openRouterChat(apiKey: string, model: string, prompt: string): Promise<string> {
  ...
  if (!resp.ok) {
    const text = await resp.text().catch(() => "");
    throw new Error(`OpenRouter error HTTP ${resp.status}${text ? `: ${text}` : ""}`);
  }
  ...
  (async () => {
    switch (msg.type) {
      ...
      case "RAG_DELETE_DOCUMENT": {
        ...
        if (!m.docId) return { ok: false, error: "Missing docId" } satisfies RagDeleteDocResponse;
      ...
      case "RAG_INDEX_DOCUMENT": {
        ...
        if (!m.apiKey) return { ok: false, error: "Missing apiKey" } satisfies RagIndexResponse;
        if (!m.doc?.id || !m.doc?.name) return { ok: false, error: "Missing doc" } satisfies RagIndexResponse;
        ...
        return { ok: true, docId: m.doc.id, chunkCount: chunkRecords.length } satisfies RagIndexResponse;
      }

      case "RAG_SEARCH": {
        const m = msg as RagSearchRequest;
        if (!m.apiKey) return { ok: false, error: "Missing apiKey" } satisfies RagSearchResponse;
        if (!m.query?.trim()) return { ok: false, error: "Missing query" } satisfies RagSearchResponse;
        const topK = Math.max(1, Math.min(20, Number(m.topK) || 6));

        const allDocs = await listDocuments();
        const docs = m.docIds && m.docIds.length ? m.docIds : allDocs.map((d) => d.id);
        if (docs.length === 0) return { ok: true, hits: [] } satisfies RagSearchResponse;
